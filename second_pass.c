...
bool add_symbols_to_code(line_info line, long *ic, machine_word **code_img, table *symbol_table) {
	...
	if (length > 1) {
		...
		analyze_operands(line, i, operands, &operand_count, NULL,params);
		...
		if (operand_count--) {
			...
			if (operand_count) {
                ...
                if((strcmp(params[0], "") != 0 && strcmp(params[1], "") != 0 )&& (get_addressing_type(params[0]) == DIRECT_ADDR || get_addressing_type(params[1]) == DIRECT_ADDR)){/*if it is JUMP ADDRESSING it adds  the required data words for the parameters */
                   if(get_addressing_type(params[0]) == DIRECT_ADDR && get_addressing_type(params[1]) == DIRECT_ADDR){   
                       isvalid = process_spass_operand(line, &curr_ic, ic, params[0], code_img, symbol_table);
                       isvalid = process_spass_operand(line, &curr_ic, ic, params[1], code_img, symbol_table);
                       free(params[1]);
                       free(params[0]);
                       }
                   else if(get_addressing_type(params[0]) == DIRECT_ADDR && get_addressing_type(params[1]) != DIRECT_ADDR){
                       isvalid = process_spass_operand(line, &curr_ic, ic, params[0], code_img, symbol_table);
                       free(params[0]);
                   } 
                   else if(get_addressing_type(params[0]) != DIRECT_ADDR && get_addressing_type(params[1]) == DIRECT_ADDR){
...
int process_spass_operand(line_info line, long *curr_ic, long *ic, char *operand, machine_word **code_img,
	...
	if (addr == JUMP_ADDR) operand++;
	if (DIRECT_ADDR == addr || JUMP_ADDR == addr) {
		long data_to_add;         
